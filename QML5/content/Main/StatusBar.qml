...
//        text: qsTr("已同步")
    ...
    function monitorSync(){
        ...
        if(scheduler.isPdbLoaded()){//db is load)
            if(passFTableModel.callIsDirty() ||  angleRelatedTableModel.callIsDirty() || motionFTableModel.callIsDirty() || controlFTableModel.callIsDirty() || comFTableModel.callIsDirty()){
